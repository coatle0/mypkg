...
vvol_wk <- function(jm,year,sigmulti){
  ...
  print(vline)
  chartSeries(ohlc_w_xts,name=jm,TA="addVo();addEMA(5,col='red');addEMA(20,col='green');addEMA(40,col='cyan');addLines(v=which(index(ohlc_w_xts) %in% vvol_evt),on=1,col='yellow')")
  dev.off()
  chartSeries(ohlc_w_xts,name=jm,TA="addVo();addEMA(5,col='red');addEMA(20,col='green');addEMA(40,col='cyan');addLines(v=which(index(ohlc_w_xts) %in% vvol_evt),on=1,col='yellow')")
